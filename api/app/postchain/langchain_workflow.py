...
from app.postchain.utils import format_stream_event # Reusing formatting if needed
...
async def run_langchain_postchain_workflow(
    ...
    # --- State Management ---
    # Simple approach: Pass message history explicitly.
    # More complex state (like phase results) can be added to a dictionary passed between steps.
...
async def main():
    ...
    async for event in run_langchain_postchain_workflow(query, thread_id, message_history, config):
