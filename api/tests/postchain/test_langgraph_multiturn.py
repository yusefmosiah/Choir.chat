...
class LangGraphMultiTurnTester:
    ...
    async def run_all_tests(self, providers: list = None) -> Dict[str, List[Dict[str, Any]]]:
        """Run specified multi-turn tests using LangGraph."""
        all_providers = {
        ...
        providers_to_run = providers or all_providers.keys()
        test_tasks = [all_providers[provider]() for provider in providers_to_run]

        all_results = await asyncio.gather(*test_tasks)

        self.results = {
            provider.capitalize(): results
            for provider, results in zip(providers_to_run, all_results)
        }
...
async def main():
    """Run the LangGraph multi-turn tests with optional provider selection."""
    parser = argparse.ArgumentParser(description='Run multi-turn conversation tests')
    parser.add_argument('--providers', nargs='+',
                        choices=['openai', 'anthropic', 'google', 'mistral', 'fireworks', 'cohere'],
                        help='List of providers to test (default: all)')
    ...
    if args.providers:
        ...
        selected_providers = [p.lower() for p in args.providers]
        await tester.run_all_tests(providers=selected_providers)
    else:
        # Run all providers by default
